...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildMetadata({
    locale: loc,
    title: t("platformMeasure.title"),
    description: t("platformMeasure.description"),
    path: "/platform/measure",
  });
}

export default async function MeasurePage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale as AppLocale);
  const loc = locale as AppLocale;
  const t = await getTranslations("platformMeasure");
  const tc = await getTranslations("breadcrumbs");

  const CRUMBS = [
    { name: tc("home"), href: "/" },
    { name: t("crumbs.platform"), href: "/#platform" },
    { name: t("crumbs.current"), href: "/platform/measure" },
  ];
  ...
  const schema = [
    breadcrumbList(CRUMBS, loc),
    {
      ...
      areaServed: ["EU", "UK"],
      url: localizedUrl("/platform/measure", loc),
        ...
        <div className="container">
          ...
          >
            <Reveal delay={1}>
              <SeeCard
                icon="chart"
                title={t("seeCards.teamTrend.title")}
                body={t("seeCards.teamTrend.body")}
                decoration={<MiniLine />}
              />
            </Reveal>
            <Reveal delay={2}>
              <SeeCard
                icon="spark"
                title={t("seeCards.variability.title")}
                body={t("seeCards.variability.body")}
            ...
            <Reveal delay={1}>
              <SeeCard
                icon="calendar"
                title={t("seeCards.annotation.title")}
                body={t("seeCards.annotation.body")}
                decoration={<MiniPins />}
              />
            </Reveal>
            <Reveal delay={2}>
              <SeeCard
                icon="scale"
                title={t("seeCards.promotion.title")}
                body={t("seeCards.promotion.body")}
          ...
          >
            <div>
              <Reveal>
                ...
                >
                  {t("promotionSection.heading")}
                </h2>
              </Reveal>
              <Reveal delay={1}>
                <p className="body" style={{ margin: 0, maxWidth: "56ch" }}>
                  {t("promotionSection.body")}
                </p>
              </Reveal>
            </div>
            <Reveal delay={2}>
        ...
        <div className="container">
          <Reveal>
            <h2
              ...
              style={{ margin: "0 0 14px", fontSize: "clamp(26px, 3.2vw, 36px)" }}
            >
              {t("employeeSection.heading")}
            </h2>
          </Reveal>
          <Reveal delay={1}>
            <p className="body" style={{ margin: "0 0 28px", maxWidth: "64ch" }}>
              {t("employeeSection.body")}
            </p>
          </Reveal>
          <Reveal delay={2}>
            <EmployeeViewCard />
        ...
        items={[
          {
            ...
            body: t("features.views.body"),
          },
          {
            icon: "calendar",
            title: t("features.annotated.title"),
            body: t("features.annotated.body"),
          },
          {
            icon: "users",
          ...
          {
            icon: "spark",
            title: t("features.variability.title"),
            body: t("features.variability.body"),
          },
          {
            icon: "scale",
            title: t("features.promotion.title"),
            body: t("features.promotion.body"),
          },
          {
            icon: "lock",
...
function TrendMockCard() {
  const t = useTranslations("platformMeasure");
      ...
      >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink-1)" }}>
                {t("trendCard.personName")}
              </div>
              <div style={{ fontSize: 11, color: "var(--ink-3)" }}>
                {t("trendCard.personRole")}
              </div>
            </div>
          </div>
          <span
        ...
        <div style={{ padding: "18px 20px 22px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {[
              { label: t("trendCard.tab90"), active: false },
              { label: t("trendCard.tab6mo"), active: false },
              { label: t("trendCard.tab12mo"), active: true },
            ].map((tab) => (
              <span
                key={tab.label}
                style={{
                  ...
                  padding: "6px 12px",
                  borderRadius: 100,
                  background: tab.active ? "var(--primary)" : "rgba(255,255,255,0.7)",
                  color: tab.active ? "white" : "var(--ink-2)",
                  border: tab.active
          ...
          >
            ...
            <svg viewBox="0 0 340 130" style={{ width: "100%", height: 150, display: "block" }}>
              ...
              {[
                t("trendCard.monthJan"),
                t("trendCard.monthApr"),
                t("trendCard.monthJul"),
                t("trendCard.monthOct"),
              ].map((m, i) => (
                <text
                  key={m}
                  x={20 + i * 100}
              ...
              <g>
                ...
                <text
                  ...
                  fill="rgba(15,29,69,0.75)"
                >
                  {t("trendCard.annotationShift")}
                </text>
              </g>
            </svg>
          </div>

          <PrivacyChip>{t("trendCard.privacyChip")}</PrivacyChip>
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {t("scoreVsTrendCompare.oneTimeScore")}
        </span>
        ...
        <div style={{ fontSize: 12.5, color: "var(--ink-3)", lineHeight: 1.55 }}>
          {t("scoreVsTrendCompare.oneTimeAffected")}
        </div>
      </div>
      ...
      >
        ...
        >
          {t("scoreVsTrendCompare.trendView")}
        </span>
        ...
        <div style={{ fontSize: 12.5, color: "var(--primary)", lineHeight: 1.55, fontWeight: 500 }}>
          {t("scoreVsTrendCompare.trendDirection")}
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        >
          {t("promotionCard.trendSlope")}
        </span>
        <span style={{ fontSize: 10, color: "var(--ink-3)" }}>{t("promotionCard.last6mo")}</span>
      </div>
      ...
      >
        ...
        />
        {t("promotionCard.signalStrongLead")} <strong>{t("promotionCard.signalStrong")}</strong>
      </div>
      ...
      >
        ...
        <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          />
          {[
            { label: t("promotionCard.roleManager"), initials: "M" },
            { label: t("promotionCard.roleHR"), initials: "HR" },
            { label: t("promotionCard.roleEmployee"), initials: "E" },
          ].map((p) => (
            <div
              key={p.label}
              style={{
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <div style={{ fontSize: 12.5, fontWeight: 600, color: "var(--ink-1)" }}>
            {t("employeeCard.yourTrendLead")}{" "}
            <span style={{ color: "var(--ink-3)", fontWeight: 500 }}>
              {t("employeeCard.yourTrendQualifier")}
            </span>
          </div>
      ...
      >
        ...
        <div>
          <div style={{ fontSize: 12.5, fontWeight: 600, color: "var(--ink-1)" }}>
            {t("employeeCard.teamAggregateLead")}{" "}
            <span style={{ color: "var(--ink-3)", fontWeight: 500 }}>
              {t("employeeCard.teamAggregateQualifier")}
            </span>
          </div>
          <svg viewBox="0 0 230 60" style={{ width: "100%", height: 56, display: "block", marginTop: 4 }}>
            <line x1="0" x2="230" y1="56" y2="56" stroke="rgba(15,29,69,0.06)" />
